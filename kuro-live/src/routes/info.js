...
const { afterPath, firstPathSegment, readSyncData, sendRouteError } = require('./utils');
...
info.get('/related/:id', async function(req, res) {
    try {
        ...
        const mal_id = readSyncData($, 'mal_id');

        const aniid = readSyncData($, 'anilist_id');
        ...
        $('.film_list-wrap').find('.flw-item').each(( index, element)=>{
            ...
            const xid = firstPathSegment($(element).find('a').attr('href'));
        ...
        sendRouteError(res, error, 'Unable to fetch anime details');
    }
});

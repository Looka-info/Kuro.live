...
src1.get('/src-server/:id', async (req, res) => {
    try {
        const servernum = numberFrom(req.params.id);
        if (!servernum) {
        ...
        const serhash = afterPath(serres?.link, '/e-1/');
        if (!serhash) {
            return res.status(502).json({ error: 'Unable to resolve source link' });
        }
        ...
        try {
            ...
            sendRouteError(res, error, 'Unable to extract stream sources');
        }
    } catch (error) {
        sendRouteError(res, error, 'Unable to fetch stream source');
    }
});

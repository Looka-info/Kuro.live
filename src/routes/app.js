...
const { firstPathSegment, sendRouteError } = require('./utils');
...
app.get('/parse', async (req, res) => {
  try {
    ...
    $('.deslide-item').each((index, element) => {
      ...
      const animeId = firstPathSegment($(element).find('.desi-buttons a:eq(1)').attr('href'));
    ...
    $('.swiper-slide.item-qtip').each(function(index,element){
      ...
      const iD = firstPathSegment($(element).find('.item a').attr('href'));
    ...
    $('.flw-item').each(function(index,element){
      ...
      const idani = firstPathSegment($(element).find('.film-name a').attr('href'));
    ...
    sendRouteError(res, error, 'Unable to parse homepage');
  }
});
